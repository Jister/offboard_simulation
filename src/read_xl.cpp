...
void data_init()
{
	...
	if(book)
	{
		if(book->load("/home/ubuntu/catkin_ws/src/offboard_simulation/data/40Hz.xls"))
		{
			...
			if(sheet)
			{
				for(int i = 0; i < 403; i++)
				{
					vx[i] = sheet->readNum(i, 3);    
					vy[i] = sheet->readNum(i, 4); 
					vz[i] = sheet->readNum(i, 5); 
				}	            
			}
		}
